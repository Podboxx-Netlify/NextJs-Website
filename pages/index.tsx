...
const Blog: React.FC = () => {
	...
	useEffect(() => {
		...
		const tags = tagFilter.length > 0 ? `&tags=${encodeURIComponent(tagFilter.join(','))}` : ''
		// Creates the Channel query
		const channel = `channel=${userState.channel || null}`
		// Changes the endpoints depending if theres any tag filter
		const endpoint = tagFilter.length > 0 ? 'tags_blog?' : 'blog?'
	...
	if (error && error.message.includes('not authorized'))
		return (
			<>
				<div className='text-2xl text-semibold mx-auto mt-10'>
					You are not authorized to view the episodes of this podcast.
				</div>
				<button
					className='btn btn-primary mt-4'
					onClick={() => router.push(userState.isLogged ? '/user/dashboard' : '/user/login')}>
					Click here to {userState.isLogged ? 'subscribe' : 'sign in'}
				</button>
			</>
		)
	if (error)
	...
	if (Object.keys(data?.podcasts).length <= 0)
		return (
			<div className='text-2xl text-semibold mx-auto mt-10 ml-5'>
				No episodes to display for this podcast.
			</div>
		)
	return (
		<>
			...
			{data?.pages > 0 && (
				<div className='py-2'>
					<ReactPaginate
						...
						nextClassName={
							'dark:text-white cursor-pointer select-none material-icons-outlined pt-0.5 hover:text-red-500'
						}
						pageClassName={
							'dark:text-white cursor-pointer select-none text-lg sm:px-3 mx-2 hover:text-red-500'
						}
