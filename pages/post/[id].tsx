...
const Post: React.FC = () => {
		...
		return (
			<div className='ml-5'>
				...
				<button
					...
					onClick={() => router.push(userState.isLogged ? '/user/subscriptions' : '/user/login')}>
	...
	return (
		<div className='w-full md:rounded-md shadow-lg flex flex-col select-none gap-4 mt-12 bg-12dp'>
			<button // Go back button
				className='text-left p-5 w-28 font-bold text-lg hover:text-red-500 focus:outline-none'
				onClick={() => {
					router.query.id !== 'preview' && router.back()
				}}>
				Go Back
			</button>
			{/* Post Content */}
			<article className='prose prose-sm lg:prose-lg mx-auto max-w-screen-lg px-5'>
				{/* Title */}
				<h1 className='text-center capitalize col-span-2'>
					{data?.title || 'Error loading the episode'}
				</h1>
				{/* Date & Duration */}
