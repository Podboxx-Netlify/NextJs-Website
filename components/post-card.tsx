...
const PostCard: React.FC<{ data: Post }> = ({ data }) => {
	...
	return (
		<>
			<div className='flex flex-col sm:flex-row p-2 w-auto lg:w-full sm:p-4 h-auto  gap-5 select-none rounded-md bg-12dp'>
				<Image
				...
				<div className='flex sm:flex-1 flex-col gap-2 p-1'>
					...
					<h1 className='text-xl line-clamp-3 overflow-hidden overflow-ellipsis'>{data.title}</h1>
					...
					<div className='flex gap-2 mt-auto whitespace-pre-wrap flex-wrap'>
									...
									<button
										...
										className={
											data.currentFilter?.includes(value)
												? 'btn btn-primary btn-xs px-2'
												: 'btn btn-secondary btn-xs px-2'
										}
						...
						<button className='btn btn-outline btn-primary ml-auto flex btn-sm'>
							<Link href={{ pathname: '/post/[id]', query: { id: data.id } }}>
								<a>View episode</a>
							</Link>
						</button>
					</div>
