...
export default function Layout({ ...props }) {
	...
	const { data } = useSWR<WebsiteData>(
		`${process.env.NEXT_PUBLIC_API_URL}${process.env.NEXT_PUBLIC_STATION_ID}/website`,
		fetcher
	)
	...
	const defaultChannel = () => {
		const channel_id = data?.channels.find((c) => c?.subscription_required === false).id
	...
	return (
		<div className='flex flex-col min-h-screen min-w-screen bg-03dp'>
			...
			<Header data={data} />
			<div className='container mx-auto flex-grow'>{props.children}</div>
			<Footer data={data} />
