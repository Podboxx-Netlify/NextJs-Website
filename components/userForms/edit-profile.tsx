...
export default function EditProfile(props) {
	...
	const updateProfile = async () => {
		...
		await Axios.put(
			`${process.env.NEXT_PUBLIC_API_URL}${userState.channel}/subscribers/auth`,
			{
	...
	return (
		<div className='grid grid-cols-1 lg:grid-cols-2'>
			<div className='lg:col-span-2'>
				{error.length > 0 &&
					error.map((e) => (
						<p key={e} className='w-full text-error whitespace-pre-line text-center'>
							{e}&nbsp;
						</p>
					))}
			</div>
			...
			<div
				className={clsx(
					'collapse rounded-box mt-5 lg:min-h-96 lg:col-span-2',
