...
export default function EditPassword() {
	...
	const updatePassword = (e) => {
		...
		auth
			...
			.catch(() => {
				ErrorNotification(
					userDispatch,
					'There was an error when editing your password.',
					'password'
				)
	...
	const verifyPassword = () => {
		const errors = []
		!validator.isStrongPassword(formData.new_password, { minSymbols: 0 }) &&
			errors.push('Password needs at least 8 characters, 1 number, 1 lowercase and 1 uppercase.')
		formData.new_password !== formData.password_confirmation &&
			errors.push('Your password confirmation does not match.')
		validator.isEmpty(formData.current_password) &&
			errors.push('Your current password cannot be blank.')
		setError(errors)
	...
	return (
		<div className='collapse-content'>
			{error.length > 0 && (
				<div className='mt-3'>
					{error.map((e) => (
						<p key={e} className='w-full text-error'>
							{e}
						</p>
					))}
				</div>
